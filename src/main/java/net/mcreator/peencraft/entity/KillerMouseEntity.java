...
public class KillerMouseEntity extends Monster {
	...
	public KillerMouseEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(PeencraftModEntities.KILLER_MOUSE.get(), world);
	}

	public KillerMouseEntity(EntityType<KillerMouseEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 1, 20) {
			...
			protected Vec3 getPosition() {
				...
				double dir_x = KillerMouseEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = KillerMouseEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = KillerMouseEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
	...
	public static void init() {
		SpawnPlacements.register(PeencraftModEntities.KILLER_MOUSE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
